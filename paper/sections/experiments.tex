...
All models are implemented in PyTorch and trained on a single NVIDIA A40 GPU (NVIDIA A40 GPU in the original paper). For Task 1, we use pre-extracted features from Omnivore and SlowFast backbones.
...
\textbf{Task 1 - Transformer:} Encoder with positional encodings processing all steps of a recording, feeding into MLP head. Trained with Adam (lr=1e-5), batch size 1, 50 epochs, weighted BCE Loss.
...
\toprule
\textbf{Split} & \textbf{Model} & \textbf{F1} & \textbf{Precision} & \textbf{Recall} & \textbf{Accuracy} & \textbf{AUC} \\
\midrule
\multicolumn{7}{@{}l}{\textit{Step-level Split}} \\
\midrule
& MLP & 0.243 & 0.661 & 0.149 & 0.711 & 0.757 \\
& Transformer & 0.554 & 0.516 & 0.598 & 0.699 & 0.756 \\
& LSTM (Ours) & 0.511 & 0.475 & 0.551 & 0.654 & 0.685 \\
\midrule
\multicolumn{7}{@{}l}{\textit{Recording-level Split}} \\
\midrule
& MLP & 0.509 & 0.453 & 0.581 & 0.598 & 0.630 \\
& Transformer & 0.390 & 0.466 & 0.336 & 0.623 & 0.623 \\
& LSTM (Ours) & 0.438 & 0.383 & 0.513 & 0.614 & 0.621 \\
\bottomrule
\end{tabular}
\end{table}
...
On recording-level split, LSTM achieves F1=0.438, outperforming Transformer (0.390) but remaining below MLP (0.509). Notably, LSTM maintains the highest accuracy (0.614) among all three architectures on this split, suggesting better generalization to video-level predictions. The consistent recall across both splits (0.551 step-level, 0.513 recording-level) indicates stable error detection capability, while precision varies more substantially (0.475 vs 0.383), reflecting the challenge of reducing false positives at different granularities.
