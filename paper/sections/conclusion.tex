...
For Task 2, we proposed a DAGNN framework modeling recipe structure through graph convolution while integrating EgoVLP visual embeddings and recipe text via Hungarian matching. Comparing against a sequential Transformer baseline using the same HiERO-extracted features, DAGNN achieved modest improvements (F1: 0.578 vs 0.505 for 8 steps; 0.587 vs 0.513 for max+1), demonstrating that explicit graph structure provides incremental benefits. However, high standard deviations (F1 std $>$ 0.15) reveal substantial variability across recipe contexts.
